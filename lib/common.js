...
var fs = require('fs');
var urllib = {};
urllib.request = require('request');
...
API.prototype.getAccessToken = function (callback) {
  ...
  urllib.request(url, {json: true}, wrapper(function (err, data) {
    if (err) {
      return callback(err);
    }
...
API.prototype._getMenu = function (callback) {
  ...
  urllib.request(url, {json: true}, wrapper(callback));
};
...
API.prototype._uploadMedia = function (filepath, type, callback) {
  ...
  fs.stat(filepath, function (err, stat) {
    ...
    form.pipe(urllib.request(url, opts, wrapper(callback)));
  });
};
